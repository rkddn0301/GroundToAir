...
public interface WishListRepository extends JpaRepository<WishListEntity, Integer> {
    ...
    @Query("SELECT w.wishNo AS wishNo, w.airlinesIata AS airlinesIata, w.departureIata AS departureIata, w.departureTime AS departureTime, w.arrivalIata AS arrivalIata, w.arrivalTime AS arrivalTime, w.flightNo AS flightNo, w.turnaroundTime AS turnaroundTime, w.stopLine AS stopLine, w.reAirlinesIata AS reAirlinesIata, w.reDepartureIata AS reDepartureIata, w.reDepartureTime AS reDepartureTime, w.reArrivalIata AS reArrivalIata, w.reArrivalTime AS reArrivalTime, w.reFlightNo AS reFlightNo, w.reTurnaroundTime AS reTurnaroundTime, w.reStopLine AS reStopLine, w.adults AS adults, w.childrens AS childrens, w.infants AS infants, w.seatClass AS seatClass, w.totalPrice AS totalPrice, w.offer AS offer, w.wishListUser.userNo AS userNo FROM WishListEntity w WHERE w.wishListUser.userNo = :userNo")
